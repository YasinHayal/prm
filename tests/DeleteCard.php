...
use Param\paramBasics\Tests\KS_Kart_Sil;
...
class DeleteCard extends Config
{
    ...
    public function send($cardGuid, $transactionId = '')
    {
        ...
        $deleteCardObj = new KS_Kart_Sil($this->clientCode,$this->clientUsername,$this->clientPassword,$cardGuid,$transactionId);
        $this->response = $client->KS_Kart_Sil($deleteCardObj);
    }
    ...
    {
        if(isset($this->response->KS_Kart_SilResult) == False){
            return [
                'Sonuc' => -2,
                'Sonuc_Str' => 'Param response has wrong format',
            ];
        }else{
            return (array)$this->response->KS_Kart_SilResult;
        }
    }
}